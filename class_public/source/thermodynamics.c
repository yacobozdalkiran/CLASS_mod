...
int thermodynamics_at_z(
  ...
  double sigmaTrescale = 1., alpha = 1., lambda_G=1., me = 1.;
  ...
  if (z >= pth->z_table[pth->tt_size-1]) {
    ...
    if (pth->has_varconst == _TRUE_) {
      class_call(background_varconst_of_z(pba, z, &alpha, &lambda_G, &me),
                 pba->error_message,
                 pth->error_message);
    ...
    if (pba->has_idm == _TRUE_) {
      ...
      if (pth->has_idm_g == _TRUE_) {
        ...
        pvecthermo[pth->index_th_dmu_idm_g] = 3./8./_PI_/(pow(pba->lambda_G,2)*_G_)*pow(1.+z, 2+pth->n_index_idm_g)*pba->Omega0_idm*pba->H0*pba->H0*pth->u_idm_g*pow(_c_,4)*_sigma_/1.e11/_eV_/_Mpc_over_m_;
      ...
      if (pth->has_idm_b == _TRUE_){
        ...
        pvecthermo[pth->index_th_R_idm_b] = (pvecback[pba->index_bg_a]*pvecback[pba->index_bg_rho_b]*pth->cross_idm_b*pth->n_coeff_idm_b/(m_b+pth->m_idm))
          ...
          *(3.e-4*pow(_c_,4.)/(8.*_PI_*_Mpc_over_m_*(pow(pba->lambda_G,2)*_G_)*_eV_)); /* conversion coefficient for the units */
...
int thermodynamics_init(
  ...
  pth->n_e = 3.*pow(pba->H0 * _c_ / _Mpc_over_m_,2)*pba->Omega0_b/(8.*_PI_*(pow(pba->lambda_G,2)*_G_)*_m_H_)*(1.-pth->YHe);
  ...
  */
  ...
  while (fgets(line,_LINE_LENGTH_MAX_-1,fA) != NULL) {
    ...
    if (left[0] > 39) {
      ...
      /* if the line contains data, we must interpret it. If (num_omegab, num_lambda_G)=(0,0), the current line must contain
         their values. Otherwise, it must contain (omegab, delatN, YHe). */
      if ((num_omegab==0) && (num_lambda_G==0)) {

        /* read (num_omegab, num_lambda_G), infer size of arrays and allocate them */
        class_test(sscanf(line,"%d %d",&num_omegab,&num_lambda_G) != 2,
                   pth->error_message,
                   "could not read value of parameters (num_omegab,num_lambda_G) in file %s\n",ppr->sBBN_file);
...
int thermodynamics_workspace_init(
  ...
  double alpha = 1., lambda_G = 1., me = 1.;
  ...
  ptw->SIunit_nH0 = 3.*ptw->SIunit_H0*ptw->SIunit_H0*pba->Omega0_b/(8.*_PI_*(pow(pba->lambda_G,2)*_G_)*_m_H_)*(1.-ptw->YHe);
  ...
  if (pth->has_varconst == _TRUE_) {
    for (index_ap=0;index_ap<ptw->ptdw->ap_size;++index_ap){
      class_call(background_varconst_of_z(pba,ptw->ptdw->ap_z_limits[index_ap], &alpha, &lambda_G, &me),
                 pba->error_message,
                 pth->error_message);
...
int thermodynamics_idm_quantities(struct background * pba,
  ...
  if (pba->has_idm == _TRUE_) {
    ...
    if (pth->has_idm_g == _TRUE_) {
      ...
      ptdw->dmu_idm_g = 3./8./_PI_/(pow(pba->lambda_G,2)*_G_)*pow(1.+z, 2+pth->n_index_idm_g)*pba->Omega0_idm*pba->H0*pba->H0*pth->u_idm_g*pow(_c_,4)*_sigma_/1.e11/_eV_/_Mpc_over_m_;
    ...
    if (pth->has_idm_b == _TRUE_) {
      ...
      ptdw->R_idm_b = (pvecback[pba->index_bg_a]*pvecback[pba->index_bg_rho_b]*pth->cross_idm_b*pth->n_coeff_idm_b/(m_b+pth->m_idm))
        *pow(T_diff_idm_b,(pth->n_index_idm_b+1.0)/2.0)*FHe
        *(3.e-4*pow(_c_,4.)/(8.*_PI_*_Mpc_over_m_*(pow(pba->lambda_G,2)*_G_)*_eV_));
