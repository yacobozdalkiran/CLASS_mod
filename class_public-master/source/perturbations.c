...
int perturbations_derivs(double tau,
  ...
  R = 4./3. * (pow(pba->lambda_G_rad,2)/pow(pba->lambda_G_m,2)) * pvecback[pba->index_bg_rho_g]/pvecback[pba->index_bg_rho_b];
  ...
  if (_scalars_) {
    ...
    if ((ppt->has_perturbed_recombination == _TRUE_)&&(ppw->approx[ppw->index_ap_tca]==(int)tca_off)){
      ...
      Nnow = 3.*H0*H0*pba->Omega0_b*(1.-pth->YHe)/(8.*_PI_*(pow(pba->lambda_G_m,2)*_G_)*_m_H_);
