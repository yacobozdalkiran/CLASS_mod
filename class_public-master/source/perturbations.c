...
int perturbations_derivs(double tau,
  ...
  if (_scalars_) {
    ...
    if ((ppt->has_perturbed_recombination == _TRUE_)&&(ppw->approx[ppw->index_ap_tca]==(int)tca_off)){
      ...
      Nnow = 3.*H0*H0*pba->Omega0_b*(1.-pth->YHe)/(8.*_PI_*(pow(pba->lambda_G_m_0,2)*_G_)*_m_H_);
...
int perturbations_tca_slip_and_shear(double * y,
  ...
  class_call(background_varconst_of_z(pba, 1./pvecback[pba->index_bg_a] - 1., &lambda_G_m, &rho_lambda, &alpha, &me),
                 pba->error_message,
                 pth->error_message);
  R = 4./3. * pvecback[pba->index_bg_rho_g]/pvecback[pba->index_bg_rho_b]*pow(pba->lambda_G_rad,2)/pow(lambda_G_m,2);
  s2_squared = 1.-3.*pba->K/k2;
