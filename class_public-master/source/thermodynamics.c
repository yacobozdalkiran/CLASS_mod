...
int thermodynamics_at_z(
  ...
  double sigmaTrescale = 1., alpha = 1., me = 1., lambda_G = 1., rho_lambda = 0.;
  ...
  if (z >= pth->z_table[pth->tt_size-1]) {
    ...
    if (pth->has_varconst == _TRUE_) {
      class_call(background_varconst_of_z(pba, z, &lambda_G, &rho_lambda, &alpha, &me),
                 pba->error_message,
                 pth->error_message);
...
int thermodynamics_init(
  ...
  pth->n_e = 3.*pow(pba->H0 * _c_ / _Mpc_over_m_,2)*pba->Omega0_b/(8.*_PI_*(pow(pba->lambda_G_0,2)*_G_)*_m_H_)*(1.-pth->YHe);
...
int thermodynamics_workspace_init(
  ...
  double alpha = 1., me = 1., lambda_G = 1., rho_lambda = 1.;
  ...
  ptw->SIunit_nH0 = 3.*ptw->SIunit_H0*ptw->SIunit_H0*pba->Omega0_b/(8.*_PI_*(pow(pba->lambda_G_0,2)*_G_)*_m_H_)*(1.-ptw->YHe);
  ...
  if (pth->has_varconst == _TRUE_) {
    for (index_ap=0;index_ap<ptw->ptdw->ap_size;++index_ap){
      class_call(background_varconst_of_z(pba,ptw->ptdw->ap_z_limits[index_ap], &lambda_G, &rho_lambda, &alpha, &me),
                 pba->error_message,
                 pth->error_message);
...
int thermodynamics_derivs(
  ...
  /* varying fundamental constants */
  double alpha = 1., me = 1., rescale_rate = 1., lambda_G = 1., rho_lambda = 0.;
  ...
  if (pth->has_varconst == _TRUE_) {
    ...
    lambda_G = pvecback[pba->index_bg_lambda_G];
    rho_lambda = pvecback[pba->index_bg_rho_lambda];
...
int thermodynamics_ionization_fractions(
  ...
  double alpha = 1., me = 1., lambda_G = 1., rho_lambda = 0.;
