...
int thermodynamics_at_z(
  ...
  double sigmaTrescale = 1., alpha = 1., me = 1., lambda_G, rho_lambda;
...
int thermodynamics_init(
  ...
  pth->n_e = 3.*pow(pba->H0 * _c_ / _Mpc_over_m_,2)*pba->Omega0_b/(8.*_PI_*(pow(pba->lambda_G_0,2)*(pow(pba->lambda_G_0,2)* _G_))*_m_H_)*(1.-pth->YHe);
...
int thermodynamics_workspace_init(
  ...
  double alpha = 1., me = 1., lambda_G, rho_lambda;
...
int thermodynamics_derivs(
  ...
  /* varying fundamental constants */
  double alpha = 1., me = 1., rescale_rate = 1., lambda_G, rho_lambda;
